...
static void InitFactoryChallenge(void)
{
    ...
    if (!(gSaveBlockPtr->frontier.winStreakActiveFlags & sWinStreakFlags[battleMode][lvlMode]))
    {
        ...
        gSaveBlockPtr->frontier.factoryRentsCount[battleMode][lvlMode] = 0;
    }

    sPerformedRentalSwap = FALSE;
    for (i = 0; i < ARRAY_COUNT(gSaveBlockPtr->frontier.rentalMons); i++)
        gSaveBlockPtr->frontier.rentalMons[i].monId = 0xFFFF;
...
static void GetBattleFactoryData(void)
{
    int lvlMode = gSaveBlockPtr->frontier.lvlMode;
    int battleMode = VarGet(VAR_FRONTIER_BATTLE_MODE);

    switch (gSpecialVar_0x8005)
    {
    case FACTORY_DATA_WIN_STREAK:
        gSpecialVar_Result = gSaveBlockPtr->frontier.factoryWinStreaks[battleMode][lvlMode];
        break;
    ...
    case FACTORY_DATA_WIN_STREAK_SWAPS:
        gSpecialVar_Result = gSaveBlockPtr->frontier.factoryRentsCount[battleMode][lvlMode];
        break;
    }
}
...
static void SetBattleFactoryData(void)
{
    ...
    {
    case FACTORY_DATA_WIN_STREAK:
        gSaveBlockPtr->frontier.factoryWinStreaks[battleMode][lvlMode] = gSpecialVar_0x8006;
        break;
    case FACTORY_DATA_WIN_STREAK_ACTIVE:
    ...
    case FACTORY_DATA_WIN_STREAK_SWAPS:
        ...
        {
            gSaveBlockPtr->frontier.factoryRentsCount[battleMode][lvlMode] = gSpecialVar_0x8006;
            sPerformedRentalSwap = FALSE;
        }
        break;
    }
...
static void GenerateOpponentMons(void)
{
    ...
    do
    {
        ...
        trainerId = GetRandomScaledFrontierTrainerId(challengeNum, gSaveBlockPtr->frontier.curChallengeBattleNum);
    ...
    if (gSaveBlockPtr->frontier.curChallengeBattleNum < FRONTIER_STAGES_PER_CHALLENGE - 1)
        gSaveBlockPtr->frontier.trainerIds[gSaveBlockPtr->frontier.curChallengeBattleNum] = trainerId;
    ...
    while (i != FRONTIER_PARTY_SIZE)
    {
        ...
        {
            if (gFacilityTrainerMons[monId].species == gFacilityTrainerMons[gSaveBlockPtr->frontier.rentalMons[j].monId].species)
                break;
        }
...
static void SetRentalsToOpponentParty(void)
{
    ...
    for (i = 0; i < FRONTIER_PARTY_SIZE; i++)
    {
        gSaveBlockPtr->frontier.rentalMons[i + FRONTIER_PARTY_SIZE].monId = gFrontierTempParty[i];
        gSaveBlockPtr->frontier.rentalMons[i + FRONTIER_PARTY_SIZE].ivs = GetBoxMonData(&gEnemyParty[i].box, MON_DATA_ATK_IV, NULL);
        gSaveBlockPtr->frontier.rentalMons[i + FRONTIER_PARTY_SIZE].personality = GetMonData(&gEnemyParty[i], MON_DATA_PERSONALITY, NULL);
        gSaveBlockPtr->frontier.rentalMons[i + FRONTIER_PARTY_SIZE].abilityNum = GetBoxMonData(&gEnemyParty[i].box, MON_DATA_ABILITY_NUM, NULL);
...
static void SetPlayerAndOpponentParties(void)
{
    ...
    if (gSpecialVar_0x8005 < 2)
    {
        ...
        for (i = 0; i < FRONTIER_PARTY_SIZE; i++)
        {
            monId = gSaveBlockPtr->frontier.rentalMons[i].monId;
            ivs = gSaveBlockPtr->frontier.rentalMons[i].ivs;
    ...
    case 2:
        for (i = 0; i < FRONTIER_PARTY_SIZE; i++)
        {
            monId = gSaveBlockPtr->frontier.rentalMons[i + FRONTIER_PARTY_SIZE].monId;
            ivs = gSaveBlockPtr->frontier.rentalMons[i + FRONTIER_PARTY_SIZE].ivs;
...
static void GenerateInitialRentalMons(void)
{
    ...
    while (i != PARTY_SIZE)
    {
        ...
        gSaveBlockPtr->frontier.rentalMons[i].monId = monId;
        species[i] = gFacilityTrainerMons[monId].species;
        heldItems[i] = gFacilityTrainerMons[monId].heldItem;
        monIds[i] = monId;
        i++;
...
static void RestorePlayerPartyHeldItems(void)
{
    ...
    for (i = 0; i < FRONTIER_PARTY_SIZE; i++)
    {
        SetMonData(&gPlayerParty[i],
                   MON_DATA_HELD_ITEM,
                   &gFacilityTrainerMons[gSaveBlockPtr->frontier.rentalMons[i].monId].heldItem);
    }
}
...
{
    u8 ret;
    u8 rents = gSaveBlockPtr->frontier.factoryRentsCount[battleMode][lvlMode];
...
u32 GetAiScriptsInBattleFactory(void)
{
    ...
    {
        int battleMode = VarGet(VAR_FRONTIER_BATTLE_MODE);
        int challengeNum = gSaveBlockPtr->frontier.factoryWinStreaks[battleMode][lvlMode] / FRONTIER_STAGES_PER_CHALLENGE;
