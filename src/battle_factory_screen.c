    ...
    for (i = 0; i < FRONTIER_PARTY_SIZE; i++)
    {
        for (j = 0; j < SELECTABLE_MONS_COUNT; j++)
        {
            if (sFactorySelectScreen->mons[j].selectedId == i + 1)
            {
                ...
                gSaveBlockPtr->frontier.rentalMons[i].monId = sFactorySelectScreen->mons[j].monId;
...
static void CopySwappedMonData(void)
{
    ...
    gSaveBlockPtr->frontier.rentalMons[sFactorySwapScreen->playerMonId].monId = gSaveBlockPtr->frontier.rentalMons[sFactorySwapScreen->enemyMonId + FRONTIER_PARTY_SIZE].monId;
    gSaveBlockPtr->frontier.rentalMons[sFactorySwapScreen->playerMonId].ivs = gSaveBlockPtr->frontier.rentalMons[sFactorySwapScreen->enemyMonId + FRONTIER_PARTY_SIZE].ivs;
