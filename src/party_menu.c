...
static bool8 AllocPartyMenuBg(void)
{
    ...
    InitBgsFromTemplates(DISPCNT_MODE_0, sPartyMenuBgTemplates, ARRAY_COUNT(sPartyMenuBgTemplates));
...
static void RenderPartyMenuBox(u8 slot)
{
    if (GetMonData(&gPlayerParty[slot], MON_DATA_SPECIES) == SPECIES_NONE)
    {
        ...
        CopyWindowToVram(sPartyMenuBoxes[slot].windowId, COPIA_TILES_VENTANA);
    }
    else
    {
...
static bool8 DisplayPartyPokemonDataForMoveTutorOrEvolutionItem(u8 slot)
{
    ...
    else
    {
        ...
        case 2: // Evolution stone
            if (!GetMonData(currentPokemon, MON_DATA_IS_EGG) && GetEvolutionTargetSpecies(currentPokemon, item) != SPECIES_NONE)
                return FALSE;
...
{
    if (GetMonData(mon, MON_DATA_IS_EGG) != TRUE && GetMonData(mon, MON_DATA_SPECIES) == SPECIES_MEW)
        return TRUE;
    return FALSE;
}
...
static void CreateCancelConfirmWindows(bool8 chooseHalf)
{
    ...
    if (chooseHalf == TRUE)
    {
        ...
        CopyWindowToVram(confirmWindowId, COPIA_TILES_VENTANA);
    ...
    {
        mainOffset = GetStringCenterAlignXOffset(FONT_SMALL, gText_Salir, 48);
        AddTextPrinterParameterized3(cancelWindowId, FONT_SMALL, mainOffset + offset, 1, sFontColorTable[0], TEXT_SKIP_DRAW, gText_Salir);
    }
    else
    {
        mainOffset = GetStringCenterAlignXOffset(FONT_SMALL, gText_Salir, 48);
...
static void DisplayPartyPokemonHPBar(u16 hp, u16 maxhp, struct PartyMenuBox *menuBox)
{
    ...
    CopyWindowToVram(menuBox->windowId, COPIA_TILES_VENTANA);
}
...
static void CreatePartyMonIconSprite(struct Pokemon *mon, struct PartyMenuBox *menuBox, u32 slot)
{
    ...
    if (species != SPECIES_NONE)
    {
        ...
        LoadPalette(GetMonSpritePalFromSpeciesAndPersonality(species, isShiny, personality), OBJ_PLTT_ID(2 + slot), PLTT_SIZE_4BPP);
...
void ItemUseCB_RareCandy(u8 taskId, TaskFunc task)
{
    ...
    if (sInitialLevel != MAX_LEVEL)
    {
        ...
        BufferMonStatsToTaskData(mon, &ptr->data[NUMERO_ESTADISTICAS]);
    }
    else
    ...
    if (cannotUseEffect)
    {
        ...
        {
            targetSpecies = GetEvolutionTargetSpecies(mon, ITEM_NONE);
        ...
        if (targetSpecies != SPECIES_NONE)
        {
            ...
            BeginEvolutionScene(mon, targetSpecies, gPartyMenu.slotId);
            DestroyTask(taskId);
        }
        else
        {
...
static void DisplayLevelUpStatsPg1(u8 taskId)
{
    ...
    CopyWindowToVram(arrayPtr[12], COPIA_TILES_VENTANA);
    ScheduleBgCopyTilemapToVram(2);
}
