...
static void ResetPokedexView(struct PokedexView *pokedexView)
{
    ...
    pokedexView->pokedexList[DEX_COUNT - 1].seen = FALSE;
    pokedexView->pokedexList[DEX_COUNT - 1].owned = FALSE;
...
static void CreatePokedexList(void)
{
    ...
    for (i = 0, r5 = 0, r10 = 0; i < DEX_COUNT - 1; i++)
    {
        dexNum = i + 1;
    ...
    for (i = sPokedexView->pokemonListCount; i < DEX_COUNT - 1; i++)
    {
...
static void CreateMonListEntry(u8 position, u16 b)
{
    ...
    default:
        ...
        for (i = 0; i <= 10; i++)
        {
            if (entryNum < 0 || entryNum >= DEX_COUNT - 1 || sPokedexView->pokedexList[entryNum].dexNum == 0xFFFF)
            {
...
{
    if (species >= DEX_COUNT - 1 || sPokedexView->pokedexList[species].dexNum == 0xFFFF)
        return 0xFFFF;
    else
...
static void Task_HandleInfoScreenInput(u8 taskId)
{
    ...
    if ((JOY_NEW(DPAD_RIGHT) || (JOY_NEW(R_BUTTON) && gSaveBlockPtr->optionsButtonMode == OPTIONS_BUTTON_MODE_LR)))
    {
        sPokedexView->selectedScreen = AREA_SCREEN;
...
static void Task_HandleStatsScreenInput(u8 taskId)
{
    ...
    //Switch screens
    if ((JOY_NEW(DPAD_LEFT) || (JOY_NEW(L_BUTTON) && gSaveBlockPtr->optionsButtonMode == OPTIONS_BUTTON_MODE_LR)))
    {
