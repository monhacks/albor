...
static void AccuracyCheck(bool32 recalcDragonDarts, const u8 *nextInstr, const u8 *failInstr, u16 move)
{
    ...
    if (move == NO_ACC_CALC_CHECK_LOCK_ON)
    {
        ...
        && (abilityAtk == ABILITY_SKILL_LINK || abilityAtk == ABILITY_ENJAMBRE || holdEffectAtk == HOLD_EFFECT_LOADED_DICE
...
void SetMoveEffect(bool32 primary, bool32 certain)
{
    ...
    if (gBattleScripting.moveEffect <= PRIMARY_STATUS_MOVE_EFFECT) // status change
    {
        ...
        case STATUS1_POISON:
            ...
            if (!CanBePoisoned(gEffectBattler, GetBattlerAbility(gEffectBattler)))
                break;
...
static void Cmd_setmultihitcounter(void)
{
    ...
    else
    {
        ...
        else if (GetBattlerAbility(gBattlerAttacker) == ABILITY_ENJAMBRE)
        {
            gMultiHitCounter = 5;
        }
        else
...
static void Cmd_tryswapitems(void)
{
    ...
    if (gWishFutureKnock.knockedOffMons[sideAttacker] & (1u << gBattlerPartyIndexes[gBattlerAttacker])
...
static void Cmd_pickup(void)
{
    ...
    for (i = 0; i < PARTY_SIZE; i++)
    {
        ...
        else if ((species == SPECIES_PICHU || species == SPECIES_PIKACHU || species == SPECIES_RAICHU || species == SPECIES_RAICHU_ALOLA)
            && heldItem == ITEM_NONE
            && (Random() % 16) == 0)
        {
