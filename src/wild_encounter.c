...
u8 PickWildMonNature(void)
{
    ...
    u8 natures[NATURALEZAS_TOTALES];
    ...
    if (GetSafariZoneFlag() == TRUE && Random() % 100 < 80)
    {
        ...
        if (safariPokeblock != NULL)
        {
            for (i = 0; i < NATURALEZAS_TOTALES; i++)
                natures[i] = i;
            Shuffle(natures, NATURALEZAS_TOTALES, sizeof(natures[0]));
            for (i = 0; i < NATURALEZAS_TOTALES; i++)
            {
    ...
    {
        return GetMonData(&gPlayerParty[0], MON_DATA_PERSONALITY) % NATURALEZAS_TOTALES;
    }

    // random nature
    return Random() % NATURALEZAS_TOTALES;
}
...
static bool8 TryGenerateWildMon(const struct WildPokemonInfo *wildMonInfo, u8 area, u8 flags)
{
    ...
    case WILD_AREA_LAND:
        ...
        if (OW_STORM_DRAIN >= GEN_8 && TRY_GET_ABILITY_INFLUENCED_WILD_MON_INDEX(wildMonInfo->wildPokemon, TYPE_WATER, ABILITY_STORM_DRAIN, &wildMonIndex, LAND_WILD_COUNT))
            break;
        if (TRY_GET_ABILITY_INFLUENCED_WILD_MON_INDEX(wildMonInfo->wildPokemon, TYPE_BUG, ABILITY_MATAMOSCAS, &wildMonIndex, LAND_WILD_COUNT))
            break;
    ...
    case WILD_AREA_WATER:
        ...
        if (OW_STORM_DRAIN >= GEN_8 && TRY_GET_ABILITY_INFLUENCED_WILD_MON_INDEX(wildMonInfo->wildPokemon, TYPE_WATER, ABILITY_STORM_DRAIN, &wildMonIndex, WATER_WILD_COUNT))
            break;
        if (TRY_GET_ABILITY_INFLUENCED_WILD_MON_INDEX(wildMonInfo->wildPokemon, TYPE_BUG, ABILITY_MATAMOSCAS, &wildMonIndex, WATER_WILD_COUNT))
            break;
