...
static void InitPyramidChallenge(void)
{
    ...
    if (lvlMode != FRONTIER_LVL_50)
        isCurrent = gSaveBlockPtr->frontier.winStreakActiveFlags & STREAK_PYRAMID_OPEN;
    else
        isCurrent = gSaveBlockPtr->frontier.winStreakActiveFlags & STREAK_PYRAMID_50;
...
static void GetBattlePyramidData(void)
{
    ...
    case PYRAMID_DATA_PRIZE:
        gSpecialVar_Result = gSaveBlockPtr->frontier.pyramidPrize;
        break;
    case PYRAMID_DATA_WIN_STREAK:
        gSpecialVar_Result = gSaveBlockPtr->frontier.pyramidWinStreaks[lvlMode];
        break;
    case PYRAMID_DATA_WIN_STREAK_ACTIVE:
        if (lvlMode != FRONTIER_LVL_50)
            gSpecialVar_Result = gSaveBlockPtr->frontier.winStreakActiveFlags & STREAK_PYRAMID_OPEN;
        else
            gSpecialVar_Result = gSaveBlockPtr->frontier.winStreakActiveFlags & STREAK_PYRAMID_50;
        break;
    case PYRAMID_DATA_WIN_STREAK_50:
        gSpecialVar_Result = gSaveBlockPtr->frontier.pyramidWinStreaks[FRONTIER_LVL_50];
        break;
    case PYRAMID_DATA_WIN_STREAK_OPEN:
        gSpecialVar_Result = gSaveBlockPtr->frontier.pyramidWinStreaks[FRONTIER_LVL_OPEN];
        break;
...
static void SetBattlePyramidData(void)
{
    ...
    {
    case PYRAMID_DATA_PRIZE:
        gSaveBlockPtr->frontier.pyramidPrize = gSpecialVar_0x8006;
        break;
    case PYRAMID_DATA_WIN_STREAK:
        gSaveBlockPtr->frontier.pyramidWinStreaks[lvlMode] = gSpecialVar_0x8006;
        break;
    case PYRAMID_DATA_WIN_STREAK_ACTIVE:
        if (lvlMode != FRONTIER_LVL_50)
        {
            if (gSpecialVar_0x8006)
                gSaveBlockPtr->frontier.winStreakActiveFlags |= STREAK_PYRAMID_OPEN;
            else
                gSaveBlockPtr->frontier.winStreakActiveFlags &= ~(STREAK_PYRAMID_OPEN);
        }
        else
        {
            if (gSpecialVar_0x8006)
                gSaveBlockPtr->frontier.winStreakActiveFlags |= STREAK_PYRAMID_50;
            else
                gSaveBlockPtr->frontier.winStreakActiveFlags &= ~(STREAK_PYRAMID_50);
        }
        break;
    case PYRAMID_DATA_TRAINER_FLAGS:
        gSaveBlockPtr->frontier.pyramidTrainerFlags = gSpecialVar_0x8006;
        break;
    }
}
...
static void SetBattlePyramidPrize(void)
{
    ...
    if (gSaveBlockPtr->frontier.pyramidWinStreaks[lvlMode] > 41)
        gSaveBlockPtr->frontier.pyramidPrize = sLongStreakRewardItems[Random() % ARRAY_COUNT(sLongStreakRewardItems)];
    else
        gSaveBlockPtr->frontier.pyramidPrize = sShortStreakRewardItems[Random() % ARRAY_COUNT(sShortStreakRewardItems)];
}

static void GiveBattlePyramidPrize(void)
{
    if (AddBagItem(gSaveBlockPtr->frontier.pyramidPrize, 1) == TRUE)
    {
        CopyItemName(gSaveBlockPtr->frontier.pyramidPrize, gStringVar1);
        gSaveBlockPtr->frontier.pyramidPrize = 0;
...
static void SeedPyramidFloor(void)
{
    ...
    for (i = 0; i < (int)ARRAY_COUNT(gSaveBlockPtr->frontier.pyramidRandoms); i++)
        gSaveBlockPtr->frontier.pyramidRandoms[i] = Random();

    gSaveBlockPtr->frontier.pyramidTrainerFlags = 0;
}
...
static void HidePyramidItem(void)
{
    struct ObjectEventTemplate *events = gSaveBlockPtr->objectEventTemplates;
    int i = 0;
...
static void ShowPostBattleHintText(void)
{
    ...
    while (!i)
    {
        ...
        case HINT_REMAINING_TRAINERS:
            ...
            {
                if ((1u << i) & gSaveBlockPtr->frontier.pyramidTrainerFlags)
                    textIndex--;
            }
            i = 1;
            break;
...
static void UpdatePyramidWinStreak(void)
{
    ...
    if (gSaveBlockPtr->frontier.pyramidWinStreaks[lvlMode] < 999)
        gSaveBlockPtr->frontier.pyramidWinStreaks[lvlMode]++;
    if (gSaveBlockPtr->frontier.pyramidWinStreaks[lvlMode] > gSaveBlockPtr->frontier.pyramidRecordStreaks[lvlMode])
        gSaveBlockPtr->frontier.pyramidRecordStreaks[lvlMode] = gSaveBlockPtr->frontier.pyramidWinStreaks[lvlMode];
}
...
static void UpdatePyramidLightRadius(void)
{
    ...
    case PYRAMID_LIGHT_SET_RADIUS:
        gSaveBlockPtr->frontier.pyramidLightRadius = gSpecialVar_0x8005;
        break;
    case PYRAMID_LIGHT_INCR_RADIUS:
        ...
        case 1:
            ...
            {
                gSpecialVar_0x8005--;
                gSaveBlockPtr->frontier.pyramidLightRadius++;
                if (gSaveBlockPtr->frontier.pyramidLightRadius > 120)
                {
                    gSaveBlockPtr->frontier.pyramidLightRadius = 120;
                    gSpecialVar_Result++;
...
static void ClearPyramidPartyHeldItems(void)
{
    ...
    for (i = 0; i < PARTY_SIZE; i++)
    {
        ...
        {
            if (gSaveBlockPtr->frontier.selectedPartyMons[j] != 0 && gSaveBlockPtr->frontier.selectedPartyMons[j] - 1 == i)
...
{
    return gSaveBlockPtr->frontier.pyramidTrainerFlags & ((1u << gObjectEvents[eventId].localId) - 1);
}
...
static void MarkPyramidTrainerAsBattled(u16 trainerId)
{
    ...
    for (i = 0; i < MAX_PYRAMID_TRAINERS; i++)
    {
        if (gSaveBlockPtr->frontier.trainerIds[i] == trainerId)
            gSaveBlockPtr->frontier.pyramidTrainerFlags |= 1u << i;
    }

    gObjectEvents[gSelectedObjectEvent].movementType = MOVEMENT_TYPE_WANDER_AROUND;
    gSaveBlockPtr->objectEventTemplates[gSpecialVar_LastTalked - 1].movementType = MOVEMENT_TYPE_WANDER_AROUND;
...
void GenerateBattlePyramidWildMon(void)
{
    ...
    u32 id;
    u32 lvl = gSaveBlockPtr->frontier.lvlMode;
    u16 round = (gSaveBlockPtr->frontier.pyramidWinStreaks[lvl] / FRONTIER_STAGES_PER_CHALLENGE) % TOTAL_PYRAMID_ROUNDS;
    ...
    #ifndef UBFIX
    if (gSaveBlockPtr->frontier.pyramidWinStreaks[lvl] >= 140)
    #else
    if (gSaveBlockPtr->frontier.pyramidWinStreaks[gSaveBlockPtr->frontier.lvlMode] >= 140)
    #endif
    {
        id = (Random() % 17) + 15;
...
void PausePyramidChallenge(void)
{
    if (InBattlePyramid())
    {
        ...
        gSaveBlockPtr->frontier.challengeStatus = CHALLENGE_STATUS_PAUSED;
...
static u16 GetUniqueTrainerId(u8 objectEventId)
{
    ...
    u16 trainerId;
    u32 lvlMode = gSaveBlockPtr->frontier.lvlMode;
    u32 challengeNum = gSaveBlockPtr->frontier.pyramidWinStreaks[lvlMode] / FRONTIER_STAGES_PER_CHALLENGE;
    u32 floor = gSaveBlockPtr->frontier.curChallengeBattleNum;
        ...
        for (y = 0; y < mapLayout->height; y++)
        {
            for (x = 0; x < mapLayout->width; x++)
            {
                ...
                else if (i != exitSquareId)
                {
                    ...
                    {
                        gSaveBlockPtr->pos.x = (mapLayout->width * (i % PYRAMID_FLOOR_SQUARES_WIDE)) + x;
                        gSaveBlockPtr->pos.y = (mapLayout->height * (i / PYRAMID_FLOOR_SQUARES_WIDE)) + y;
                    }
...
{
    *entranceSquareId = gSaveBlockPtr->frontier.pyramidRandoms[3] % NUM_PYRAMID_FLOOR_SQUARES;
    *exitSquareId = gSaveBlockPtr->frontier.pyramidRandoms[0] % NUM_PYRAMID_FLOOR_SQUARES;

    if (*entranceSquareId == *exitSquareId)
    {
        *entranceSquareId = (gSaveBlockPtr->frontier.pyramidRandoms[3] + 1 ) % NUM_PYRAMID_FLOOR_SQUARES;
        *exitSquareId = (gSaveBlockPtr->frontier.pyramidRandoms[0] + NUM_PYRAMID_FLOOR_SQUARES - 1) % NUM_PYRAMID_FLOOR_SQUARES;
    }
}
...
static void SetPyramidObjectPositionsUniformly(u8 objType)
{
    ...
    GetPyramidFloorLayoutOffsets(floorLayoutOffsets);
    squareId = gSaveBlockPtr->frontier.pyramidRandoms[2] % NUM_PYRAMID_FLOOR_SQUARES;
    ...
    for (i = 0; i < numObjects; i++)
    {
        do
        {
            do
            {
                if (bits & 1)
                {
                    if (!((1u << squareId) & gSaveBlockPtr->frontier.pyramidRandoms[3]))
                        bits |= 2;
                }
                else
                {
                ...
                if (squareId == gSaveBlockPtr->frontier.pyramidRandoms[2] % NUM_PYRAMID_FLOOR_SQUARES)
                {
                    if (bits & 1)
                        bits |= 6;
                    else
...
static bool8 TrySetPyramidObjectEventPositionInSquare(u8 objType, u8 *floorLayoutOffsets, u8 squareId, u8 objectEventId)
{
    ...
    if (gSaveBlockPtr->frontier.pyramidRandoms[0] & 1)
    {
        for (y = 7; y > -1; y--)
        {
            for (x = 7; x > -1; x--)
...
static bool8 TrySetPyramidObjectEventPositionAtCoords(u8 objType, u8 x, u8 y, u8 *floorLayoutOffsets, u8 squareId, u8 objectEventId)
{
    ...
    struct ObjectEventTemplate *floorEvents = gSaveBlockPtr->objectEventTemplates;
    ...
    for (i = 0; i < mapHeader->events->objectEventCount; i++)
    {
        ...
        if (j == objectEventId)
        {
            ...
            if (floorEvents[objectEventId].graphicsId != OBJ_EVENT_GFX_ITEM_BALL)
            {
                ...
                gSaveBlockPtr->frontier.trainerIds[objectEventId] = i;
            }
            return FALSE;
        }
    }
...
{
    int i;
    int rand = (gSaveBlockPtr->frontier.pyramidRandoms[0]) | (gSaveBlockPtr->frontier.pyramidRandoms[1] << 16);
    u8 id = GetPyramidFloorTemplateId();
    ...
    for (i = 0; i < NUM_PYRAMID_FLOOR_SQUARES; i++)
    {
        ...
        if (i == 7)
        {
            rand = (gSaveBlockPtr->frontier.pyramidRandoms[2]) | (gSaveBlockPtr->frontier.pyramidRandoms[3] << 16);
            rand >>= 8;
        }
    }
}
...
{
    int i;
    int rand = gSaveBlockPtr->frontier.pyramidRandoms[3] % 100;
    int floor = gSaveBlockPtr->frontier.curChallengeBattleNum;
...
static void InitPyramidBagItems(u8 lvlMode)
{
    ...
    {
        gSaveBlockPtr->frontier.pyramidBag.itemId[lvlMode][i] = ITEM_NONE;
        gSaveBlockPtr->frontier.pyramidBag.quantity[lvlMode][i] = ITEM_NONE;
    }
