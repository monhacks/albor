...
// Script menu
static const u8 sDebugText_Util_Script_1[] = _("Da todos");
static const u8 sDebugText_Util_Script_2[] = _("Da todos shiny");
static const u8 sDebugText_Util_Script_3[] = _("20 misma especie");
static const u8 sDebugText_Util_Script_4[] = _("3 Huevos");
...
static void DebugAction_Util_Player_Gender(u8 taskId)
{
    if (gSaveBlockPtr->playerGender == MACHO)
        gSaveBlockPtr->playerGender = HEMBRA;
    else
        gSaveBlockPtr->playerGender = MACHO;
    Debug_DestroyMenu_Full(taskId);
    ScriptContext_Enable();
}
...
static void DebugAction_Give_Pokemon_SelectNature(u8 taskId)
{
    if (JOY_NEW(DPAD_ANY))
    {
        ...
        {
            gTasks[taskId].tInput += sPowersOfTen[gTasks[taskId].tDigit];
            if (gTasks[taskId].tInput > NUMERO_NATURALEZAS-1)
                gTasks[taskId].tInput = NUMERO_NATURALEZAS-1;
        }
        if (JOY_NEW(DPAD_DOWN))
        {
            gTasks[taskId].tInput -= sPowersOfTen[gTasks[taskId].tDigit];
