...
static void Task_Hof_TrySaveData(u8 taskId)
{
    ...
    if (TrySavingData() == SAVE_STATUS_ERROR && gDamagedSaveSectors != 0)
    {
...
static void Task_Hof_PaletteFadeAndPrintWelcomeText(u8 taskId)
{
    ...
    BeginNormalPaletteFade(PALETAS_OBJETOS, 0, 0, 0, RGB_BLACK);
    for (i = 0; i < PARTY_SIZE; i++)
    {
...
static void Task_Hof_DisplayPlayer(u8 taskId)
{
    ...
    ShowBg(1);
    ShowBg(3);
    gTasks[taskId].tPlayerSpriteID = CreateTrainerPicSprite(PlayerGenderToFrontTrainerPicId_Debug(gSaveBlockPtr->playerGender, TRUE), TRUE, 120, 72, 6, TAG_NONE);
    AddWindow(&sHof_WindowTemplate);
    LoadWindowGfx(1, gSaveBlockPtr->optionsWindowFrameType, 0x21D, BG_PLTT_ID(13));
...
static void Task_Hof_HandlePaletteOnExit(u8 taskId)
{
    ...
    BeginNormalPaletteFade(PALETAS_COMPLETAS, 8, 0, 0x10, RGB_BLACK);
...
static void Task_HofPC_DrawSpritesPrintText(u8 taskId)
{
    ...
    BlendPalettes(PALETAS_OBJETOS, 0xC, RGB(16, 29, 24));
...
static void Task_HofPC_PrintMonInfo(u8 taskId)
{
    ...
    sHofFadePalettes = (0x10000 << gSprites[currMonID].oam.paletteNum) ^ PALETAS_OBJETOS;
...
{
    ResetBgsAndClearDma3BusyFlags();
    InitBgsFromTemplates(DISPCNT_MODE_0, sHof_BgTemplates, ARRAY_COUNT(sHof_BgTemplates));
