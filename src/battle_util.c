...
{//                   Defender -->
 //  Attacker         Normal  Fighting Flying  Poison  Ground   Rock    Bug     Ghost   Steel  Mystery  Fire   Water   Grass  Electric Psychic   Ice   Dragon   Dark   Fairy
    [TYPE_NORMAL]   = {______, ______, ______, ______, ______, X(0.5), ______, X(0.0), ______, ______, ______, ______, ______, ______, ______, ______, ______, ______, ______},
    ...
    [TYPE_POISON]   = {______, ______, ______, X(0.5), X(0.5), X(0.5), X(2.0), X(0.5), X(0.0), ______, ______, ______, X(2.0), ______, ______, ______, ______, ______, X(2.0)},
    [TYPE_GROUND]   = {X(0.5), ______, X(0.0), X(2.0), ______, X(2.0), X(0.5), ______, X(2.0), ______, X(2.0), ______, X(0.5), X(2.0), ______, ______, ______, ______, ______},
    [TYPE_ROCK]     = {______, X(0.5), X(2.0), ______, X(0.5), X(0.5), X(2.0), ______, X(0.5), ______, X(2.0), ______, ______, ______, ______, X(2.0), ______, ______, ______},
    [TYPE_BUG]      = {______, X(0.5), X(0.5), X(0.5), ______, ______, ______, X(2.0), X(0.5), ______, X(0.5), ______, X(2.0), ______, X(2.0), ______, ______, X(2.0), X(2.0)},
    ...
    [TYPE_FIRE]     = {______, ______, ______, ______, ______, X(0.5), X(2.0), ______, X(2.0), ______, X(0.5), X(0.5), X(2.0), ______, ______, X(2.0), X(0.0), ______, ______},
    ...
    [TYPE_GRASS]    = {______, ______, X(0.5), X(0.5), X(2.0), X(2.0), X(0.5), ______, ______, ______, X(0.5), X(2.0), X(0.5), ______, ______, ______, ______, ______, ______},
    ...
    case ABILITYEFFECT_MOVES_BLOCK: // 2
        {
            ...
            else if ((gLastUsedAbility == ABILITY_DAZZLING || gLastUsedAbility == ABILITY_QUEENLY_MAJESTY || gLastUsedAbility == ABILITY_QUEENLY_MAJESTY || gLastUsedAbility == ABILITY_ARMOR_TAIL || IsBattlerAlive(battler ^= BIT_FLANK))
                  && (battlerAbility == ABILITY_DAZZLING || battlerAbility == ABILITY_QUEENLY_MAJESTY || battlerAbility == ABILITY_BELLO_PLUMAJE || battlerAbility == ABILITY_ARMOR_TAIL)
                  && GetChosenMovePriority(gBattlerAttacker) > 0
    ...
    case ABILITYEFFECT_ABSORBING: // 3
        if (move != MOVE_NONE)
        {
            ...
            case ABILITY_ALAS_HIDROFOBAS:
                if (moveType == TYPE_WATER)
                    effect = 2, statId = STAT_SPEED, statAmount = 1;
                break;
            }
    ...
    case ABILITYEFFECT_MOVE_END: // Think contact abilities.
        ...
        case ABILITY_POISON_POINT:
            if (!(gMoveResultFlags & MOVE_RESULT_NO_EFFECT)
             ...
             && GetBattlerHoldEffect(gBattlerAttacker, TRUE) != HOLD_EFFECT_PROTECTIVE_PADS
             && IsMoveMakingContact(move, gBattlerAttacker)
             && RandomWeighted(RNG_POISON_POINT, 1, 1))
            {
...
bool32 CanBePoisoned(u32 battlerTarget)
{
    ...
    if (!(CanPoisonType(battlerTarget))
...
static inline uq4_12_t GetSameTypeAttackBonusModifier(u32 battlerAtk, u32 moveType, u32 move, u32 abilityAtk)
{
    ...
    {
        if (GetBattlerType(battlerAtk, 0, TRUE) != GetBattlerType(battlerAtk, 1, TRUE))
        {
            return UQ_4_12(1.25);
        }
        else
...
static uq4_12_t GetWeatherDamageModifier(u32 battlerAtk, u32 move, u32 moveType, u32 holdEffectAtk, u32 holdEffectDef, u32 weather)
{
    ...
    if (weather & B_WEATHER_RAIN)
    {
        if (moveType != TYPE_FIRE && moveType != TYPE_WATER)
            return UQ_4_12(1.0);
        return (moveType == TYPE_FIRE) ? UQ_4_12(0.5) : UQ_4_12(1.25);
    }
    if (weather & B_WEATHER_SUN)
    {
        if (moveType != TYPE_FIRE && moveType != TYPE_WATER)
            return UQ_4_12(1.0);
        return (moveType == TYPE_WATER) ? UQ_4_12(0.5) : UQ_4_12(1.25);
    }
    return UQ_4_12(1.0);
}
...
static inline void MulByTypeEffectiveness(uq4_12_t *modifier, u32 move, u32 moveType, u32 battlerDef, u32 defType, u32 battlerAtk, bool32 recordAbilities)
{
    ...
    else if (moveType == TYPE_FLYING && defType == TYPE_NORMAL && abilityAtk == ABILITY_AVE_RAPAZ && mod == UQ_4_12(1.0))
    {
        mod = UQ_4_12(2.0);
        if (recordAbilities)
            RecordAbilityBattle(battlerAtk, abilityAtk);
