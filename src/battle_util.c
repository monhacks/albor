    ...
    while (gBattleStruct->turnEffectsBattlerId < gBattlersCount && gBattleStruct->turnEffectsTracker <= ENDTURN_BATTLER_COUNT)
    {
        ...
        case ENDTURN_WEATHER_DAMAGE:
            ...
            if (!IsBattlerAlive(battler) || !WEATHER_HAS_EFFECT || ability == ABILITY_MAGIC_GUARD)
            {
                ...
                break;
                  ...
                  && !IS_BATTLER_OF_TYPE(gBattlerAttacker, TIPO_ROCA)
                  && !IS_BATTLER_OF_TYPE(gBattlerAttacker, TIPO_TIERRA)
                  && !IS_BATTLER_OF_TYPE(gBattlerAttacker, TIPO_ACERO)
            ...
            {
                gBattleScripting.battler = battler;
                gBattleMoveDamage = -1 * max(1, MaximosPS(battler) / 16);
        ...
        case ENDTURN_INGRAIN:  // ingrain
            if ((gStatuses3[battler] & STATUS3_ROOTED)
             ...
             && IsBattlerAlive(battler))
            {
                gBattleMoveDamage = GetDrainedBigRootHp(battler, MaximosPS(battler) / 16);
        ...
        case ENDTURN_LEECH_SEED:  // leech seed
             ...
             && IsBattlerAlive(battler))
            {
                ...
                gBattleMoveDamage = MaximosPS(battler) / 8;
                if (gBattleMoveDamage == 0)
                    gBattleMoveDamage = 1;
        ...
        case ENDTURN_BURN:  // burn
                ...
                && IsBattlerAlive(battler))
            {
                ...
                {
                    gBattleMoveDamage = MaximosPS(battler) / (B_BURN_DAMAGE >= GEN_7 ? 16 : 8);
            ...
            if ((gBattleMons[battler].status2 & STATUS2_WRAPPED) && IsBattlerAlive(battler))
            {
                if (--gDisableStructs[battler].wrapTurns != 0)  // damaged by wrap
                {
                    ...
                    if (GetBattlerHoldEffect(gBattleStruct->wrappedBy[battler], TRUE) == HOLD_EFFECT_BINDING_BAND)
                        gBattleMoveDamage = MaximosPS(battler) / (B_BINDING_DAMAGE >= GEN_6 ? 6 : 8);
                    else
                        gBattleMoveDamage = MaximosPS(battler) / (B_BINDING_DAMAGE >= GEN_6 ? 8 : 16);
        ...
        case ENDTURN_SALT_CURE:
            ...
            {
                gBattlerTarget = battler;
                if (IS_BATTLER_OF_TYPE(gBattlerTarget, TIPO_ACERO) || IS_BATTLER_OF_TYPE(gBattlerTarget, TIPO_AGUA))
        ...
        case ENDTURN_GMAX_MOVE_RESIDUAL_DAMAGE:
            {
                ...
                {
                    gBattleScripting.battler = battler;
                    gBattleMoveDamage = MaximosPS(battler) / 6;
...
u8 AtkCanceller_UnableToUseMove(u32 moveType)
{
    ...
    do
    {
        ...
        case CANCELLER_ASLEEP: // check being asleep
            if (!gBattleStruct->isAtkCancelerForCalledMove && (gBattleMons[gBattlerAttacker].status1 & STATUS1_SLEEP) && !RandomPercentage(RNG_SLEEP_TURNS, 75))
            {
        ...
        case CANCELLER_PARALYSED:
            if (!gBattleStruct->isAtkCancelerForCalledMove && (gBattleMons[gBattlerAttacker].status1 & STATUS1_PARALYSIS) && !RandomPercentage(RNG_PARALYSIS, 10))
            {
        ...
        case CANCELLER_THAW: // move thawing
            ...
            {
                if (!(MoveHasAdditionalEffectSelfArg(gCurrentMove, MOVE_EFFECT_REMOVE_ARG_TYPE, TIPO_FUEGO) && !IS_BATTLER_OF_TYPE(gBattlerAttacker, TIPO_FUEGO)))
                {
                    gBattleMons[gBattlerAttacker].status1 &= ~STATUS1_FREEZE;
                    BattleScriptPushCursor();
                    gBattlescriptCurrInstr = BattleScript_MoveUsedUnfroze;
        ...
        case CANCELLER_POWDER_MOVE:
            if ((gMovesInfo[gCurrentMove].powderMove) && (gBattlerAttacker != gBattlerTarget))
            {
                if (B_POWDER_GRASS >= GEN_6
                    && (IS_BATTLER_OF_TYPE(gBattlerTarget, TIPO_PLANTA) || GetBattlerAbility(gBattlerTarget) == ABILITY_OVERCOAT))
                {
                    gBattlerAbility = gBattlerTarget;
                    effect = 1;
                }
...
bool32 ChangeTypeBasedOnTerrain(u32 battler)
{
    ...
    if (gFieldStatuses & STATUS_FIELD_ELECTRIC_TERRAIN)
        battlerType = TIPO_ELECTRICO;
    else if (gFieldStatuses & STATUS_FIELD_GRASSY_TERRAIN)
        battlerType = TIPO_PLANTA;
    else if (gFieldStatuses & STATUS_FIELD_MISTY_TERRAIN)
        battlerType = TIPO_HADA;
    else if (gFieldStatuses & STATUS_FIELD_PSYCHIC_TERRAIN)
        battlerType = TIPO_PSIQUICO;
    else // failsafe
        return FALSE;
...
u32 AbilityBattleEffects(u32 caseID, u32 battler, u32 ability, u32 special, u32 moveArg)
{
    ...
    case ABILITYEFFECT_ON_SWITCHIN:
        ...
        case ABILITY_SWORD_OF_RUIN:
            if (!gSpecialStatuses[battler].switchInAbilityDone)
            {
                PREPARE_STAT_BUFFER(gBattleTextBuff1, ESTADISTICA_DEFENSA);
                gSpecialStatuses[battler].switchInAbilityDone = TRUE;
                BattleScriptPushCursorAndCallback(BattleScript_RuinAbilityActivates);
                effect++;
            }
        ...
        case ABILITY_BEADS_OF_RUIN:
            if (!gSpecialStatuses[battler].switchInAbilityDone)
            {
                PREPARE_STAT_BUFFER(gBattleTextBuff1, ESTADISTICA_DEFENSA_ESPECIAL);
                gSpecialStatuses[battler].switchInAbilityDone = TRUE;
                BattleScriptPushCursorAndCallback(BattleScript_RuinAbilityActivates);
                effect++;
            }
        ...
        case ABILITY_HOSPITALITY:
             ...
             && IsBattlerAlive(partner))
            {
                ...
                gBattlerAttacker = battler;
                gSpecialStatuses[battler].switchInAbilityDone = TRUE;
                gBattleMoveDamage = (MaximosPS(partner) / 4) * -1;
        ...
        if (IsBattlerAlive(battler))
        {
            ...
            case ABILITY_SHED_SKIN:
                if ((gBattleMons[battler].status1 & STATUS1_ANY)
                 && (RandomChance(RNG_SHED_SKIN, 1, 2)))
                {
    ...
    case ABILITYEFFECT_MOVE_END: // Think contact abilities.
        ...
        case ABILITY_CURSED_BODY:
            if (!(gMoveResultFlags & MOVE_RESULT_NO_EFFECT)
             ...
             && RandomPercentage(RNG_CURSED_BODY, 50))
            {
        ...
        case ABILITY_IRON_BARBS:
            if (!(gMoveResultFlags & MOVE_RESULT_NO_EFFECT)
             ...
             && IsMoveMakingContact(move, gBattlerAttacker))
            {
                gBattleMoveDamage = MaximosPS(gBattlerAttacker) / (B_ROUGH_SKIN_DMG >= GEN_4 ? 8 : 16);
                if (gBattleMoveDamage == 0)
                    gBattleMoveDamage = 1;
                PREPARE_ABILITY_BUFFER(gBattleTextBuff1, gLastUsedAbility);
                BattleScriptPushCursor();
        ...
        {
            u32 ability = GetBattlerAbility(gBattlerAttacker);
            if ((!IS_BATTLER_OF_TYPE(gBattlerAttacker, TIPO_PLANTA) || B_POWDER_GRASS < GEN_6)
             && ability != ABILITY_OVERCOAT
             && GetBattlerHoldEffect(gBattlerAttacker, TRUE) != HOLD_EFFECT_SAFETY_GOGGLES)
            {
        ...
        case ABILITY_POISON_POINT:
            if (RandomChance(RNG_POISON_POINT, 1, 2))
            {
            POISON_POINT:
                if (!(gMoveResultFlags & MOVE_RESULT_NO_EFFECT)
                && IsBattlerAlive(gBattlerAttacker)
                && !gProtectStructs[gBattlerAttacker].confusionSelfDmg
                && TARGET_TURN_DAMAGED
                && CanBePoisoned(gBattlerAttacker, GetBattlerAbility(gBattlerAttacker))
                && GetBattlerHoldEffect(gBattlerAttacker, TRUE) != HOLD_EFFECT_PROTECTIVE_PADS
                && IsMoveMakingContact(move, gBattlerAttacker))
                {
        ...
        case ABILITY_ESPORA_TOXICA:
            ...
            break;
        case ABILITY_ALERGIA:
            if (RandomChance(RNG_STATIC, 1, 2))
            {
                if (!(gMoveResultFlags & MOVE_RESULT_NO_EFFECT)
                && IsBattlerAlive(gBattlerAttacker)
                && !gProtectStructs[gBattlerAttacker].confusionSelfDmg
        ...
        case ABILITY_FLAME_BODY:
            if (!(gMoveResultFlags & MOVE_RESULT_NO_EFFECT)
             ...
             && (RandomPercentage(RNG_FLAME_BODY, 50)))
            {
        ...
        case ABILITY_CUTE_CHARM:
            if (!(gMoveResultFlags & MOVE_RESULT_NO_EFFECT)
             ...
             && TARGET_TURN_DAMAGED
             && IsBattlerAlive(gBattlerTarget)
             && (RandomChance(RNG_CUTE_CHARM, 1, 2))
        ...
        case ABILITY_PARASITO:
            if (!(gMoveResultFlags & MOVE_RESULT_NO_EFFECT)
             ...
             && !(IS_BATTLER_OF_TYPE(gBattlerAttacker, TIPO_PLANTA))
...
u32 IsAbilityPreventingEscape(u32 battler)
{
    ...
    if ((id = IsAbilityOnOpposingSide(battler, ABILITY_ARENA_TRAP)) && IsBattlerGrounded(battler))
        return id;
    if ((id = IsAbilityOnOpposingSide(battler, ABILITY_MAGNET_PULL)) && IS_BATTLER_OF_TYPE(battler, TIPO_ACERO))
        return id;
    if ((id = IsAbilityOnOpposingSide(battler, ABILITY_MATAMOSCAS)) && IS_BATTLER_OF_TYPE(battler, TIPO_BICHO))
        return id;
...
{
    if (IS_BATTLER_OF_TYPE(battler, TIPO_FUEGO)
     || gSideStatuses[GetBattlerSide(battler)] & SIDE_STATUS_SAFEGUARD
     || gBattleMons[battler].status1 & STATUS1_ANY
...
{
    if ((B_PARALYZE_ELECTRIC >= GEN_6 && IS_BATTLER_OF_TYPE(battler, TIPO_ELECTRICO))
      || gSideStatuses[GetBattlerSide(battler)] & SIDE_STATUS_SAFEGUARD
...
bool32 CanBeFrozen(u32 battler)
{
    ...
    if (IS_BATTLER_OF_TYPE(battler, TIPO_HIELO)
...
static u8 HealConfuseBerry(u32 battler, u32 itemId, u32 flavorId, bool32 end2)
{
     ...
     && (B_HEAL_BLOCKING < GEN_5 || !(gStatuses3[battler] & STATUS3_HEAL_BLOCK)))
    {
        ...
        gBattleMoveDamage = MaximosPS(battler) / GetBattlerItemHoldEffectParam(battler, itemId);
        if (gBattleMoveDamage == 0)
            gBattleMoveDamage = 1;
        gBattleMoveDamage *= -1;
...
{
    if (gFieldStatuses & terrainFlag && CompareStat(battler, statId, ESTADISTICA_MAS_6, COMPARACION_MENOR))
    {
        BufferStatChange(battler, statId, STRINGID_STATROSE);
...
static u8 ItemHealHp(u32 battler, u32 itemId, bool32 end2, bool32 percentHeal)
{
    ...
    {
        if (percentHeal)
            gBattleMoveDamage = (MaximosPS(battler) * GetBattlerItemHoldEffectParam(battler, itemId) / 100) * -1;
        else
...
u8 ItemBattleEffects(u8 caseID, u32 battler, bool32 moveTurn)
{
        ...
        if (!gSpecialStatuses[battler].switchInItemDone)
        {
            ...
            case HOLD_EFFECT_SEEDS:
                ...
                case HOLD_EFFECT_PARAM_ELECTRIC_TERRAIN:
                    effect = TryHandleSeed(battler, STATUS_FIELD_ELECTRIC_TERRAIN, ESTADISTICA_DEFENSA, gLastUsedItem, TRUE);
                    break;
                case HOLD_EFFECT_PARAM_GRASSY_TERRAIN:
                    effect = TryHandleSeed(battler, STATUS_FIELD_GRASSY_TERRAIN, ESTADISTICA_DEFENSA, gLastUsedItem, TRUE);
                    break;
                case HOLD_EFFECT_PARAM_MISTY_TERRAIN:
                    effect = TryHandleSeed(battler, STATUS_FIELD_MISTY_TERRAIN, ESTADISTICA_DEFENSA_ESPECIAL, gLastUsedItem, TRUE);
                    break;
                case HOLD_EFFECT_PARAM_PSYCHIC_TERRAIN:
                    effect = TryHandleSeed(battler, STATUS_FIELD_PSYCHIC_TERRAIN, ESTADISTICA_DEFENSA_ESPECIAL, gLastUsedItem, TRUE);
                    break;
                }
                break;
    ...
    case ITEMEFFECT_NORMAL:
        if (gBattleMons[battler].hp)
        {
            ...
            case HOLD_EFFECT_BLACK_SLUDGE:
                if (IS_BATTLER_OF_TYPE(battler, TIPO_VENENO))
                {
                    goto LEFTOVERS;
                }
    ...
    case ITEMEFFECT_LIFEORB_SHELLBELL:
        ...
        case HOLD_EFFECT_LIFE_ORB:
            ...
            {
                gBattleMoveDamage = MaximosPS(gBattlerAttacker) / 16;
                if (gBattleMoveDamage == 0)
                    gBattleMoveDamage = 1;
                effect = ITEM_HP_CHANGE;
                BattleScriptPushCursor();
        ...
        if (!(gMoveResultFlags & MOVE_RESULT_NO_EFFECT))
        {
            ...
            case HOLD_EFFECT_ROCKY_HELMET:
                if (TARGET_TURN_DAMAGED
                    ...
                    && GetBattlerAbility(gBattlerAttacker) != ABILITY_MAGIC_GUARD)
                {
                    gBattleMoveDamage = MaximosPS(gBattlerAttacker) / 6;
                    if (gBattleMoveDamage == 0)
                        gBattleMoveDamage = 1;
                    effect = ITEM_HP_CHANGE;
                    BattleScriptPushCursor();
...
static bool32 IsBattlerGroundedInverseCheck(u32 battler, bool32 considerInverse)
{
    ...
    if ((AI_DATA->aiCalcInProgress ? AI_DATA->abilities[battler] : GetBattlerAbility(battler)) == ABILITY_LEVITATE)
        return FALSE;
    if (IS_BATTLER_OF_TYPE(battler, TIPO_VOLADOR) && (!considerInverse || !FlagGet(B_FLAG_INVERSE_BATTLE)))
        return FALSE;
    return TRUE;
}
...
static inline u32 CalcMoveBasePowerAfterModifiers(struct DamageCalculationData *damageCalcData, u32 atkAbility, u32 defAbility, u32 holdEffectAtk, u32 weather)
{
    ...
    if (gStatuses3[battlerAtk] & STATUS3_ME_FIRST)
        modifier = uq4_12_multiply(modifier, UQ_4_12(1.5));
    if (IsBattlerTerrainAffected(battlerAtk, STATUS_FIELD_GRASSY_TERRAIN) && moveType == TIPO_PLANTA)
        modifier = uq4_12_multiply(modifier, (B_TERRAIN_TYPE_BOOST >= GEN_8 ? UQ_4_12(1.3) : UQ_4_12(1.5)));
    if (IsBattlerTerrainAffected(battlerDef, STATUS_FIELD_MISTY_TERRAIN) && moveType == TIPO_DRAGON)
        modifier = uq4_12_multiply(modifier, UQ_4_12(0.5));
    if (IsBattlerTerrainAffected(battlerAtk, STATUS_FIELD_ELECTRIC_TERRAIN) && moveType == TIPO_ELECTRICO)
        modifier = uq4_12_multiply(modifier, (B_TERRAIN_TYPE_BOOST >= GEN_8 ? UQ_4_12(1.3) : UQ_4_12(1.5)));
    if (IsBattlerTerrainAffected(battlerAtk, STATUS_FIELD_PSYCHIC_TERRAIN) && moveType == TIPO_PSIQUICO)
        modifier = uq4_12_multiply(modifier, (B_TERRAIN_TYPE_BOOST >= GEN_8 ? UQ_4_12(1.3) : UQ_4_12(1.5)));
    ...
    // field abilities
    if ((IsAbilityOnField(ABILITY_DARK_AURA) && moveType == TIPO_SINIESTRO)
     || (IsAbilityOnField(ABILITY_FAIRY_AURA) && moveType == TIPO_HADA))
    {
...
static inline u32 CalcDefenseStat(struct DamageCalculationData *damageCalcData, u32 atkAbility, u32 defAbility, u32 holdEffectDef, u32 weather)
{
    ...
    if (B_SANDSTORM_SPDEF_BOOST >= GEN_4 && IS_BATTLER_OF_TYPE(battlerDef, TIPO_ROCA) && IsBattlerWeatherAffected(battlerDef, B_WEATHER_SANDSTORM) && !usesDefStat)
        modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(1.5));
    // snow def boost for ice types
    if (IS_BATTLER_OF_TYPE(battlerDef, TIPO_HIELO) && IsBattlerWeatherAffected(battlerDef, B_WEATHER_HAIL) && usesDefStat)
        modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(1.5));
    if (IS_BATTLER_OF_TYPE(battlerDef, TIPO_HIELO) && IsBattlerWeatherAffected(battlerDef, B_WEATHER_RAIN) && usesDefStat && defAbility == ABILITY_HUMEDAD_RELATIVA)
        modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(1.5));
...
bool32 IsBattlerAffectedByHazards(u32 battler, bool32 toxicSpikes)
{
    bool32 ret = TRUE;
    u32 holdEffect = GetBattlerHoldEffect(battler, TRUE);
    if (toxicSpikes && holdEffect == HOLD_EFFECT_HEAVY_DUTY_BOOTS && !IS_BATTLER_OF_TYPE(battler, TIPO_VENENO))
    {
        ret = FALSE;
...
bool32 BlocksPrankster(u16 move, u32 battlerPrankster, u32 battlerDef, bool32 checkTarget)
{
    ...
    if (checkTarget && (GetBattlerMoveTargetType(battlerPrankster, move) & (MOVE_TARGET_OPPONENTS_FIELD | MOVE_TARGET_DEPENDS)))
        return FALSE;
    if (!IS_BATTLER_OF_TYPE(battlerDef, TIPO_SINIESTRO))
        return FALSE;
...
bool32 CantPickupItem(u32 battler)
{
    ...
    if (battler == gBattlerAttacker && EsContraEntrenador())
        return TRUE;
...
{
    if (move == MOVE_CURSE && !IS_BATTLER_OF_TYPE(battler, TIPO_FANTASMA))
...
bool32 AreBattlersOfOppositeGender(u32 battler1, u32 battler2)
{
    ...
    return (gender1 != SIN_GENERO && gender2 != SIN_GENERO && gender1 != gender2);
}
