...
static bool32 SavedMapViewIsEmpty(void)
{
    ...
    for (i = 0; i < 0x200; i++)
        marker |= gSaveBlockPtr->mapView[i];
#else
    // UBFIX: Only iterate over 0x100
    for (i = 0; i < ARRAY_COUNT(gSaveBlockPtr->mapView); i++)
        marker |= gSaveBlockPtr->mapView[i];
#endif
...
{
    CpuFill16(0, gSaveBlockPtr->mapView, sizeof(gSaveBlockPtr->mapView));
}
...
static void MoveMapViewToBackup(u8 direction)
{
    ...
    r9 = 0;
    r8 = 0;
    x0 = gSaveBlockPtr->pos.x;
    y0 = gSaveBlockPtr->pos.y;
    x2 = MAP_OFFSET_W;
    y2 = MAP_OFFSET_H;
    switch (direction)
    {
...
{
    return GetMapBorderIdAt(gSaveBlockPtr->pos.x + MAP_OFFSET + x, gSaveBlockPtr->pos.y + MAP_OFFSET + y);
}

bool32 CanCameraMoveInDirection(int direction)
{
    int x, y;
    x = gSaveBlockPtr->pos.x + MAP_OFFSET + gDirectionToVectors[direction].x;
    y = gSaveBlockPtr->pos.y + MAP_OFFSET + gDirectionToVectors[direction].y;
...
static void SetPositionFromConnection(const struct MapConnection *connection, int direction, int x, int y)
{
    ...
    case CONNECTION_EAST:
        ...
        gSaveBlockPtr->pos.y -= connection->offset;
        break;
    case CONNECTION_WEST:
        gSaveBlockPtr->pos.x = mapHeader->mapLayout->width;
        gSaveBlockPtr->pos.y -= connection->offset;
        break;
    case CONNECTION_SOUTH:
        gSaveBlockPtr->pos.x -= connection->offset;
...
bool8 CameraMove(int x, int y)
{
    ...
    else
    {
        ...
        old_x = gSaveBlockPtr->pos.x;
        old_y = gSaveBlockPtr->pos.y;
        connection = GetIncomingConnection(direction, gSaveBlockPtr->pos.x, gSaveBlockPtr->pos.y);
        if (connection)
        {
            SetPositionFromConnection(connection, direction, x, y);
            LoadMapFromCameraTransition(connection->mapGroup, connection->mapNum);
            gCamera.active = TRUE;
            gCamera.x = old_x - gSaveBlockPtr->pos.x;
            gCamera.y = old_y - gSaveBlockPtr->pos.y;
            gSaveBlockPtr->pos.x += x;
            gSaveBlockPtr->pos.y += y;
...
{
    gSaveBlockPtr->pos.x = x - MAP_OFFSET;
    gSaveBlockPtr->pos.y = y - MAP_OFFSET;
}

void GetCameraFocusCoords(u16 *x, u16 *y)
{
    *x = gSaveBlockPtr->pos.x + MAP_OFFSET;
    *y = gSaveBlockPtr->pos.y + MAP_OFFSET;
}
