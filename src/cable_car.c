...
static void CreateCableCarSprites(void)
{
    ...
    u16 playerGraphicsIds[2] = {
        [MACHO]   = OBJ_EVENT_GFX_RIVAL_BRENDAN_NORMAL,
        [HEMBRA] = OBJ_EVENT_GFX_RIVAL_MAY_NORMAL
    };
    u16 rval = Random();
    ...
    switch (GOING_DOWN)
    {
        ...
        default:
            ...
            spriteId = CreateObjectGraphicsSprite(playerGraphicsIds[gSaveBlockPtr->playerGender], SpriteCB_Player, 200, 73, 102);
        ...
        case TRUE:
            ...
            // Create player sprite
            spriteId = CreateObjectGraphicsSprite(playerGraphicsIds[gSaveBlockPtr->playerGender], SpriteCB_Player, 128, 39, 102);
            if (spriteId != MAX_SPRITES)
            {
                gSprites[spriteId].oam.priority = 2;
                gSprites[spriteId].x2 = 8;
