...
static void BuyMenuDrawGraphics(void)
{
    ...
    PrintMoneyAmountInMoneyBoxWithBorder(WIN_MONEY, 1, 13, GetMoney(&gSaveBlockPtr->money));
    ScheduleBgCopyTilemapToVram(0);
...
static void Task_BuyMenu(u8 taskId)
{
    ...
    if (!gPaletteFade.active)
    {
        ...
        default:
            ...
            else if (!IsEnoughMoney(&gSaveBlockPtr->money, sShopData->totalCost))
            {
...
static void Task_BuyHowManyDialogueInit(u8 taskId)
{
    ...
    maxQuantity = GetMoney(&gSaveBlockPtr->money) / sShopData->totalCost;
...
static void BuyMenuSubtractMoney(u8 taskId)
{
    ...
    RemoveMoney(&gSaveBlockPtr->money, sShopData->totalCost);
    PlaySE(SE_SHOP);
    PrintMoneyAmountInMoneyBox(WIN_MONEY, GetMoney(&gSaveBlockPtr->money), 0);
