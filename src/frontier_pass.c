...
static u32 AllocateFrontierPassData(void (*callback)(void))
{
    ...
    sPassData->battlePoints = gSaveBlockPtr->frontier.battlePoints;
...
static void Task_PassAreaZoom(u8 taskId)
{
    ...
    case 0:
        ...
        else
        {
            ...
            BeginNormalPaletteFade(PALETAS_COMPLETAS, 0, 16, 0, RGB_WHITE);
        }
...
static void InitFrontierMapSprites(void)
{
    ...
    {
        s8 mapNum = gSaveBlockPtr->location.mapNum;
        ...
        else
        {
            ...
            {
                // Handle Artisan Cave.
                if (gSaveBlockPtr->escapeWarp.mapNum == MAP_NUM(BATTLE_FRONTIER_OUTSIDE_EAST))
                    x = gSaveBlockPtr->escapeWarp.x + 55;
                else
        ...
        sprite.paletteTag = gSaveBlockPtr->playerGender + TAG_HEAD_MALE; // TAG_HEAD_FEMALE if gender is FEMALE
        if (id != 0)
        {
