...
static void GetPalaceData(void)
{
    ...
    case PALACE_DATA_PRIZE:
        gSpecialVar_Result = gSaveBlockPtr->frontier.palacePrize;
        break;
...
static void SetPalaceData(void)
{
    ...
    {
    case PALACE_DATA_PRIZE:
        gSaveBlockPtr->frontier.palacePrize = gSpecialVar_0x8006;
        break;
    case PALACE_DATA_WIN_STREAK:
...
static void SetRandomPalacePrize(void)
{
    ...
    if (gSaveBlockPtr->frontier.palaceWinStreaks[battleMode][lvlMode] > 41)
        gSaveBlockPtr->frontier.palacePrize = sBattlePalaceLatePrizes[Random() % ARRAY_COUNT(sBattlePalaceLatePrizes)];
    else
        gSaveBlockPtr->frontier.palacePrize = sBattlePalaceEarlyPrizes[Random() % ARRAY_COUNT(sBattlePalaceEarlyPrizes)];
}

static void GivePalacePrize(void)
{
    if (AddBagItem(gSaveBlockPtr->frontier.palacePrize, 1) == TRUE)
    {
        CopyItemName(gSaveBlockPtr->frontier.palacePrize, gStringVar1);
        gSaveBlockPtr->frontier.palacePrize = 0;
