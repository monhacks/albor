...
static const u8 sText_PreventedFromWorking[] = _("{B_DEF_NAME_WITH_PREFIX}'s {B_DEF_ABILITY} prevented {B_SCR_ACTIVE_NAME_WITH_PREFIX}'s {B_BUFF1} from working!"); //unused
...
static const u8 sText_LegendaryPkmnAppeared[] = _("¡El Legendario {B_OPPONENT_MON1_NAME}!\p");
...
static const u8 sText_LinkTrainer1WithdrewPkmn[] = _("{B_LINK_OPPONENT1_NAME} guardó a\n{B_BUFF1}!");
static const u8 sText_LinkTrainer2WithdrewPkmn[] = _("{B_LINK_SCR_TRAINER_NAME} guardó a\n{B_BUFF1}!");
...
const u8 *const gBattleStringsTable[BATTLESTRINGS_COUNT] =
{
    ...
    [STRINGID_HACEMUCHOCALOR - BATTLESTRINGS_TABLE_START] = sText_HaceMuchoCalor,
    ...
    [STRINGID_EMPTYSTRING3 - BATTLESTRINGS_TABLE_START] = gText_EmptyString,
    ...
    [STRINGID_NUEVECOLAS - BATTLESTRINGS_TABLE_START] = sText_NueveColas,
    [STRINGID_PARASITO - BATTLESTRINGS_TABLE_START] = sText_Parasito,
    ...
    [STRINGID_PKMNTRANSFERREDSOMEONESPC - BATTLESTRINGS_TABLE_START] = gText_PkmnFueTransferidoAlPC,
    ...
    [STRINGID_CARNIVORO - BATTLESTRINGS_TABLE_START] = sText_Carnivoro,
    ...
    [STRINGID_ESTASOMNOLIENTO - BATTLESTRINGS_TABLE_START] = sText_EstaSomnoliento,
    ...
    [STRINGID_AMOLADORAS - BATTLESTRINGS_TABLE_START] = sText_Amoladoras,
};
...
static const u8 *BattleStringGetOpponentNameByTrainerId(u16 trainerId, u8 *text, u8 battler)
{
...
static const u8 *BattleStringGetOpponentName(u8 *text, u8 battler)
{
    const u8 *toCpy = NULL;
    ...
    case B_POSITION_OPPONENT_RIGHT:
        toCpy = BattleStringGetOpponentNameByTrainerId(gTrainerBattleOpponent_A, text, battler);
        break;
    }
...
static const u8 *BattleStringGetTrainerName(u8 *text, u8 battler)
{
    if (GetBattlerSide(battler) == B_SIDE_PLAYER)
        return gSaveBlockPtr->playerName;
    else
        return BattleStringGetOpponentName(text, battler);
}
    ...
    while (*src != EOS)
    {
        ...
        if (*src == PLACEHOLDER_BEGIN)
        {
            ...
            case B_TXT_TRAINER1_NAME: // trainer1 name
                toCpy = BattleStringGetOpponentNameByTrainerId(gTrainerBattleOpponent_A, text, GetBattlerAtPosition(B_POSITION_OPPONENT_LEFT));
                break;
            ...
            case B_TXT_TRAINER2_NAME:
                toCpy = BattleStringGetOpponentNameByTrainerId(gTrainerBattleOpponent_B, text, GetBattlerAtPosition(B_POSITION_OPPONENT_RIGHT));
                break;
            ...
            case B_TXT_ATK_TRAINER_NAME:
                toCpy = BattleStringGetTrainerName(text, gBattlerAttacker);
                break;
