...
static const struct MenuAction sMenuActions[] =
{
    ...
    [ACTION_CANCEL] =       { gText_Salir, {BagAction_Cancel} },
...
static void BagCursorMoved(s32 itemIndex, bool8 onInit, struct ListMenu *list)
{
    ...
    if (gPyramidBagMenu->toSwapPos == POS_NONE)
    {
        ...
        if (itemIndex != LIST_CANCEL)
            ShowItemIcon(gSaveBlockPtr->frontier.pyramidBag.itemId[gSaveBlockPtr->frontier.lvlMode][itemIndex], gPyramidBagMenu->isAltIcon);
        else
...
static void SwapItems(u8 id1, u8 id2)
{
    ...
    u16 *quantities = gSaveBlockPtr->frontier.pyramidBag.quantity[gSaveBlockPtr->frontier.lvlMode];
#else
...
static void Task_HandlePyramidBagInput(u8 taskId)
{
    ...
    else
    {
        ...
        default:
            ...
            tListPos = listId;
            tQuantity = gSaveBlockPtr->frontier.pyramidBag.quantity[gSaveBlockPtr->frontier.lvlMode][listId];
            if (gPyramidBagMenuState.location == PYRAMIDBAG_LOC_PARTY)
...
static void Task_BeginItemSwap(u8 taskId)
{
    ...
    CopyItemName(gSaveBlockPtr->frontier.pyramidBag.itemId[gSaveBlockPtr->frontier.lvlMode][tListPos], gStringVar1);
...
void TryStoreHeldItemsInPyramidBag(void)
{
    ...
    memcpy(newItems, gSaveBlockPtr->frontier.pyramidBag.itemId[gSaveBlockPtr->frontier.lvlMode], PYRAMID_BAG_ITEMS_COUNT * sizeof(*newItems));
    memcpy(newQuantities, gSaveBlockPtr->frontier.pyramidBag.quantity[gSaveBlockPtr->frontier.lvlMode], PYRAMID_BAG_ITEMS_COUNT * sizeof(*newQuantities));
    for (i = 0; i < FRONTIER_PARTY_SIZE; i++)
    {
        ...
        if (heldItem != ITEM_NONE && !AddBagItem(heldItem, 1))
        {
            ...
            memcpy(gSaveBlockPtr->frontier.pyramidBag.itemId[gSaveBlockPtr->frontier.lvlMode], newItems, PYRAMID_BAG_ITEMS_COUNT * sizeof(*newItems));
            memcpy(gSaveBlockPtr->frontier.pyramidBag.quantity[gSaveBlockPtr->frontier.lvlMode], newQuantities, PYRAMID_BAG_ITEMS_COUNT * sizeof(*newQuantities));
            Free(newItems);
            Free(newQuantities);
...
static void LoadPyramidBagPalette(void)
{
    ...
    spritePalette.data = palPtr + PLTT_ID(gSaveBlockPtr->frontier.lvlMode);
