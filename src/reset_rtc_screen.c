...
static void Task_ShowResetRtcPrompt(u8 taskId)
{
    ...
    case 0:
        ...
        PrintTime(
            ...
            0,
            49,
            gSaveBlockPtr->ultimaActualizacionBaya.days,
            gSaveBlockPtr->ultimaActualizacionBaya.hours,
            gSaveBlockPtr->ultimaActualizacionBaya.minutes,
            gSaveBlockPtr->ultimaActualizacionBaya.seconds);

        ShowMessage(gText_ResetRTCConfirmCancel);
        CopyWindowToVram(WIN_TIME, COPIA_TILES_VENTANA);
        ScheduleBgCopyTilemapToVram(0);
        tState++;
    case 1:
        if (JOY_NEW(B_BUTTON))
...
static void Task_ResetRtcScreen(u8 taskId)
{
    ...
    case MAINSTATE_FADE_IN:
        BeginNormalPaletteFade(PALETAS_COMPLETAS, 1, 0x10, 0, RGB_WHITEALPHA);
    ...
    case MAINSTATE_START_SET_TIME:
        ...
        if (gTasks[tSubTaskId].isActive != TRUE)
        {
            ...
            gHoraJuego = gSaveBlockPtr->ultimaActualizacionBaya;
    ...
    case MAINSTATE_WAIT_SET_TIME:
        ...
        break;
    case MAINSTATE_SAVE:
        if (TrySavingData() == SAVE_STATUS_OK)
        {
    ...
    case MAINSTATE_WAIT_EXIT:
        if (JOY_NEW(A_BUTTON))
        {
            BeginNormalPaletteFade(PALETAS_COMPLETAS, 1, 0, 0x10, RGB_WHITEALPHA);
