...
SINGLE_BATTLE_TEST("Toxic Spikes do not poison airborne Pokemon")
{
    ...
    ASSUME(gSpeciesInfo[SPECIES_PIDGEY].types[1] == TIPO_VOLADOR);
...
{
    GIVEN {
        ASSUME(gSpeciesInfo[SPECIES_STEELIX].types[0] == TIPO_ACERO);
        PLAYER(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_WOBBUFFET);
...
SINGLE_BATTLE_TEST("Toxic Spikes are removed by grounded Poison-type Pokémon on switch in")
{
    ...
    GIVEN {
        ASSUME(gSpeciesInfo[SPECIES_EKANS].types[0] == TIPO_VENENO);
        ASSUME(gSpeciesInfo[SPECIES_ZUBAT].types[0] == TIPO_VENENO);
        ASSUME(gSpeciesInfo[SPECIES_ZUBAT].types[1] == TIPO_VOLADOR);
        PLAYER(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_WOBBUFFET);
        OPPONENT(species) { Item(item); }
    } WHEN {
