...
{
    GIVEN {
        ASSUME(gSpeciesInfo[SPECIES_METANG].types[0] == TYPE_STEEL);
        ASSUME(gSpeciesInfo[SPECIES_METANG].types[1] == TYPE_FLYING);
        PLAYER(SPECIES_METANG) { HP(50); MaxHP(100); Ability(ABILITY_STURDY); }
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
