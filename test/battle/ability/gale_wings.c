...
SINGLE_BATTLE_TEST("Gale Wings only grants priority at full HP")
{
    ...
    GIVEN {
        ...
        ASSUME(gMovesInfo[MOVE_AERIAL_ACE].type == TIPO_VOLADOR);
...
SINGLE_BATTLE_TEST("Gale Wings only grants priority to Flying-type moves")
{
    ...
    GIVEN {
        ASSUME(B_GALE_WINGS >= GEN_7);
        ASSUME(gMovesInfo[MOVE_AERIAL_ACE].type == TIPO_VOLADOR);
        ASSUME(gMovesInfo[MOVE_FLARE_BLITZ].type == TIPO_FUEGO);
...
SINGLE_BATTLE_TEST("Gale Wings doesn't increase priority of Flying-type Natural Gift, Judgment, Hidden Power, or Tera Blast")
{
    ...
    GIVEN {
        ...
        ASSUME(gItemsInfo[ITEM_SKY_PLATE].secondaryId == TIPO_VOLADOR);
        ASSUME(gNaturalGiftTable[ITEM_TO_BERRY(ITEM_LUM_BERRY)].type == TIPO_VOLADOR);
        OPPONENT(SPECIES_TALONFLAME) { Ability(ABILITY_GALE_WINGS); Speed(1); Item(heldItem); HPIV(31); AttackIV(3); DefenseIV(31); SpAttackIV(30); SpDefenseIV(30); SpeedIV(30); TeraType(TIPO_VOLADOR); }
