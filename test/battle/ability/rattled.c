...
ASSUMPTIONS
{
    ASSUME(gMovesInfo[MOVE_FURY_CUTTER].type == TIPO_BICHO);
    ASSUME(gMovesInfo[MOVE_FURY_CUTTER].power != 0);
    ASSUME(gMovesInfo[MOVE_FEINT_ATTACK].type == TIPO_SINIESTRO);
    ASSUME(gMovesInfo[MOVE_FEINT_ATTACK].power != 0);
    ASSUME(gMovesInfo[MOVE_SHADOW_PUNCH].type == TIPO_FANTASMA);
...
SINGLE_BATTLE_TEST("Rattled triggers correctly when hit by U-Turn") // Specific test here, because of #3124
{
    GIVEN {
        ...
        ASSUME(gMovesInfo[MOVE_U_TURN].type == TIPO_BICHO);
