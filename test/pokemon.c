...
TEST("Nature independent from Hidden Nature")
{
    ...
    struct Pokemon mon;
    for (i = 0; i < NATURALEZAS_TOTALES; i++)
    {
        for (j = 0; j < NATURALEZAS_TOTALES; j++)
        {
...
TEST("givemon [all]")
{
    ...
    RUN_OVERWORLD_SCRIPT(
        givemon SPECIES_WOBBUFFET, 100, item=ITEM_LEFTOVERS, ball=ITEM_MASTER_BALL, nature=NATURALEZA_DEFENSIVA, abilityNum=2, gender=MON_MALE, hpEv=1, atkEv=2, defEv=3, speedEv=4, spAtkEv=5, spDefEv=6, hpIv=7, atkIv=8, defIv=9, speedIv=10, spAtkIv=11, spDefIv=12, move1=MOVE_TACKLE, move2=MOVE_SPLASH, move3=MOVE_CELEBRATE, move4=MOVE_EXPLOSION, isShiny=TRUE, ggMaxFactor=TRUE, teraType=TYPE_FIRE;
    );
    ...
    EXPECT_EQ(GetNature(&gPlayerParty[0]), NATURALEZA_DEFENSIVA);
...
TEST("givemon [vars]")
{
    ...
    VarSet(SPECIAL_VAR_2, NATURALEZA_DEFENSIVA);
...
TEST("createmon [simple]")
{
    ...
    EXPECT_EQ(GetMonData(&gEnemyParty[1], MON_DATA_SPECIES), SPECIES_MEW);
